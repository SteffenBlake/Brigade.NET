...
/// <summary>Shared lexical group discovery for transport generators; unrelated containers add no scope.</summary>
public static class RouteGroupHierarchy
{
    /// <summary>Gets the enclosing groups, outermost first, including the supplied type if it is a group.</summary>
    ...
    public static IEnumerable<AttributeData> GetAttributes(IMethodSymbol route) =>
        GetGroups(route.ContainingType).SelectMany(group => group.GetAttributes()).Concat(route.GetAttributes());
