...
internal static class HandlerRouteDeclarations
{
    ...
    private static bool ReferencesContracts(IAssemblySymbol assembly, CancellationToken token)
    {
        ...
        while (pending.Count != 0)
        {
            ...
            if (current.GetTypeByMetadataName(
                "Brigade.Net.Partie.Engines.AspNetCore.IQueryRoutePolicy`1") is not null
                || current.GetTypeByMetadataName(
                    "Brigade.Net.Partie.Engines.AspNetCore.ICommandRoutePolicy`1") is not null)
            {
                return true;
            }
    ...
    {
        var queryContract = compilation.GetTypeByMetadataName(
            "Brigade.Net.Partie.Engines.AspNetCore.IQueryRoutePolicy`1");
        var commandContract = compilation.GetTypeByMetadataName(
            "Brigade.Net.Partie.Engines.AspNetCore.ICommandRoutePolicy`1");
        var contracts = type.AllInterfaces.Where(candidate =>
            SymbolEqualityComparer.Default.Equals(candidate.OriginalDefinition, queryContract)
            || SymbolEqualityComparer.Default.Equals(candidate.OriginalDefinition, commandContract)).ToArray();
        if (queryContract is null || commandContract is null || type.TypeKind != TypeKind.Class
            || type.IsAbstract || type.IsFileLocal || contracts.Length == 0
            || !compilation.IsSymbolAccessibleWithin(type, compilation.Assembly))
        {
            return null;
        }
        ...
        for (var current = type; current is not null; current = current.ContainingType)
        {
            if ((current != type && current.Arity != 0) || current.IsFileLocal)
            {
                return null;
            }
        }

        if (type.TypeParameters.Any(parameter => !contracts.Any(contract =>
            contract.TypeArguments.Any(argument => Contains(argument, parameter)))))
        {
            return null;
        }
        ...
        source.Append("[global::Brigade.Net.Partie.Engines.AspNetCore.RoutePolicy(typeof(")
            .Append(UnboundTypeName(type)).Append("))]\n")
    ...
    private static string UnboundTypeName(INamedTypeSymbol type)
    {
        ...
        return name.Substring(0, name.IndexOf('<')) + "<" + new string(',', type.Arity - 1) + ">";
    }

    private static bool Contains(ITypeSymbol type, ITypeParameterSymbol parameter)
    {
        return SymbolEqualityComparer.Default.Equals(type, parameter)
            || type is IArrayTypeSymbol array && Contains(array.ElementType, parameter)
            || type is INamedTypeSymbol named && named.TypeArguments.Any(argument => Contains(argument, parameter));
    }
}
