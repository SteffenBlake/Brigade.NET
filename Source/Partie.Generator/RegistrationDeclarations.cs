...
public static class RegistrationDeclarations
{
    public static GeneratedDeclaration? Describe(INamedTypeSymbol type, Compilation compilation)
    {
        var contract = type.AllInterfaces.FirstOrDefault(candidate =>
            candidate.OriginalDefinition.Equals(compilation.GetTypeByMetadataName("Brigade.Net.Partie.IPartie`2"),
                SymbolEqualityComparer.Default));
        if (contract is null)
        {
            return null;
        }
        var provider = type.AllInterfaces.Any(candidate =>
            candidate.OriginalDefinition.Equals(compilation.GetTypeByMetadataName("Brigade.Net.Partie.IProvider`2"),
                SymbolEqualityComparer.Default));
