...
public static class RoutePipelineEmitter
{
    ...
    {
        var resultType =
            "global::Brigade.Net.Core.Results.Result<" + TypeName(graph.ResultType) + ">";
                ...
                .WithParameterList(
                    ParameterList(
                        SingletonSeparatedList(
                            Parameter(Identifier(ValueName(output))).WithType(
                                ParseTypeName(TypeName(output.Type))
                            )
                        )
                    )
                )
        ...
        return MethodDeclaration(returnType, methodName)
            ...
            .WithParameterList(
                ParameterList(
                    SeparatedList(
                        graph.ExternalValues.Select(value =>
                            Parameter(Identifier(ValueName(value))).WithType(
                                ParseTypeName(TypeName(value.Type))
                            )
                        )
                    )
                )
        ...
        ExpressionSyntax Invoke(RouteCall call, string? continuationName = null)
        {
            ...
            if (call.Method.Arity != 0)
            {
                name = GenericName(Identifier("@" + call.Method.Name))
                    .WithTypeArgumentList(
                        TypeArgumentList(
                            SeparatedList(
                                call.Method.TypeArguments.Select(type => ParseTypeName(TypeName(type)))
                            )
                        )
                    );
            }
            ...
            {
                var argumentName = parameter.Ordinal == call.ContinuationParameterIndex
                    ? continuationName!
            ...
            ExpressionSyntax invocation = InvocationExpression(
                MemberAccessExpression(
                    ...
                    ParseName(TypeName(call.Method.ContainingType)),
                    name
                ),
            ...
            if (
                continuationName is null
                && call.Method.ReturnType is INamedTypeSymbol named
                && (named.MetadataName == "Result`1" || named.MetadataName == "Task`1")
            )
            {
