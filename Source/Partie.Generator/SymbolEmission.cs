...
public static class SymbolEmission
{
    ...
    public static string Metadata(ISymbol symbol, Func<AttributeData, bool>? omit = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(xml))
        {
            ...
            docs = string.Join(
                "\n",
                string.Join("\n", root.Nodes().Select(node => node.ToString()))
                    .Split('\n')
                    .Select(line => "/// " + line)
            ) + "\n";
        }
        ...
        return docs + string.Join(
            ...
            symbol.GetAttributes()
                .Where(attribute =>
                    omit?.Invoke(attribute) != true && !IsCompilerMetadata(attribute)
                )
                .Select(attribute =>
                    "["
                    ...
                    + string.Join(
                        ...
                        attribute.ConstructorArguments.Select(Constant).Concat(
                            attribute.NamedArguments.Select(argument =>
                                "@" + argument.Key + " = " + Constant(argument.Value)
                            )
                        )
                    )
                    + ")]"
    ...
    private static bool IsCompilerMetadata(AttributeData attribute) =>
        attribute.AttributeClass?.ToDisplayString()
            is "System.Runtime.CompilerServices.NullableAttribute"
                ...
                or "System.Runtime.CompilerServices.DynamicAttribute"
    ...
    public static string Constant(TypedConstant value)
    {
        ...
        {
            return "new "
                + TypeName(((IArrayTypeSymbol)value.Type!).ElementType)
                + "[] { "
                + string.Join(", ", value.Values.Select(Constant))
                + " }";
        }
        ...
        var literal = SymbolDisplay.FormatPrimitive(value.Value!, true, false)
            ?? throw new InvalidOperationException("Unsupported attribute constant.");
        return value.Type?.SpecialType
            is SpecialType.System_String or SpecialType.System_Boolean or SpecialType.System_Char
            ? literal
            : "(" + TypeName(value.Type!) + ")(" + literal + ")";
    }
}
