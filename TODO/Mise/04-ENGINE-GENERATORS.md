...
- [ ] Implement one incremental generator per engine: SQL Server, PostgreSQL, SQLite, MySQL, and MariaDB. Each generator passes its distinctly named runtime table and row attributes into shared parsing and owns dialect quoting and capability checks.
...
- [ ] Multi-engine consumer tests install more than one runtime/analyzer pair, prove each table and row target triggers only its matching generator, and assert diagnostics for conflicting engine markers on one type.
