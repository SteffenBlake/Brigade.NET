...
- [ ] Test const identifiers inside attributes, engine-owned schemas, aliases and alias-qualified relationship constants, every supported join, CTE/recursive CTE, correlated and set queries, parameters/raw diagnostics, CRUD, generated values, composite keys, all row shapes, transactions, `FirstOrNotFound`, cancellation, and `NULL` mismatch.
