...
- 2026-09-23 | execution | ITxn, UnitOfWork, and BasicTxn accept cancellation tokens; commit gets the route token, while automatic rollback gets a bounded cleanup token | a canceled route token must not prevent rollback from starting | 06,07
- 2026-09-24 | execution | UnitOfWork asynchronously disposes its ITxn children after commit or rollback; a shared DbWriter uses the same provider transaction but does not own that transaction | keep transaction alive through Partie outcome handling and dispose it once | 06,07
- 2026-09-24 | example | Each of five engine routes selects its own config through a route provider and uses distinct generated Tbl identifiers for the same demanding logical query; Aspire starts five migrated databases through engine-specific Liquibase changelogs | prove provider isolation, generated API use, and cross-engine behavior with near-identical schemas | 07,08
- 2026-09-24 | partie | Named config provider reads IConfiguration connection strings and resolves a keyed DbProviderFactory with the same ServiceNames key | route-specific configs must select both their connection and ADO.NET engine without ambiguous exact-type service resolution | 07,08
- 2026-09-24 | example | Keep mapped Tbl types, requests, and handlers in Example.Domain slices; keep Aspire/provider registration and Partie routes in Example.Web/Routing.V1.cs. Read handlers return the `IReadOnlyList<T>` from `DbReader.ListAsync` directly | match the established vertical-slice layout and avoid needless result copies | 07,08
